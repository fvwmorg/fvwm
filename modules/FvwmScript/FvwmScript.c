...
void Xinit(int IsFather)
{
  ...
  if (IsFather)
  {
    name = (char*)xcalloc(sizeof(char),strlen("FvwmScript")+5);
    do
    {
...
void OpenWindow (void)
{
  ...
  Scrapt = (char*)xcalloc(sizeof(char),1);
...
void BuildGUI(int IsFather)
{
  ...
  for (i=0; i<nbobj; i++)
  {
    tabxobj[i] = (struct XObj*)xcalloc(1,sizeof(struct XObj));
...
void ReadXServer (void)
{
  ...
  while (FEventsQueued(dpy, QueuedAfterReading))
  {
	...
	octet = XFetchBuffer(dpy,&i,7);
      ...
      if (i > 0)
      {
	Scrapt = (char*)xrealloc((void*)Scrapt,sizeof(char)*(i+1), sizeof(char *));
...
void ReadFvwmScriptArg(int argc, char **argv,int IsFather)
{
  ...
  {                             /* Cas du fils */
    x11base->TabScriptId[0] = (char*)xcalloc(sizeof(char),strlen(argv[7]));
...
int main (int argc, char **argv)
{
  ...
  x11base = (X11base*) xcalloc(1,sizeof(X11base));
