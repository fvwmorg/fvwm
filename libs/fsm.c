...
unique_filename(char *path, char *prefix, int *pFd)
{
	...
	tempFile = safemalloc(strlen(path) + strlen(prefix) + 8);
	...
	int count,FIceListenObj *listenObjs,FIceAuthDataEntry **authDataEntries)
{
	...
	*authDataEntries = safemalloc(count * 2 * sizeof (FIceAuthDataEntry));
	...
	FIcePointer *watch_data)
{
	...
	if (opening)
	{
		fice_conn = safemalloc(sizeof *fice_conn);
...
int fsm_init(char *module)
{
	...
	ice_fd = safemalloc(sizeof *ice_fd * numTransports + 1);
	...
	p = safemalloc(16 + strlen(networkIds) + 1);
